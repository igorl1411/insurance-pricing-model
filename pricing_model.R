...
#Mean Absolute Error
prognozy_portfela <- predict(model_glm, type = "response")
bledy_wyceny <- abs(df$charges - prognozy_portfela)
sred_blad <- mean(bledy_wyceny)
cat("Średni błąd wyceny polisy (MAE) dla modelu GLM wynosi:", round(sred_blad), "USD")
